def scrape_data():
    ...
    uri = f'postgresql://postgres:{password}@localhost:5432/final_project'
    engine = create_engine(uri)

    # Check for last read season
    dbLstSeason = pd.read_sql('select MAX(season) as lstseaon from matches_final', con=engine)
    dbLastseason = pd.read_sql('select * from matches_final', con=engine)
    dbLastseason['curr_year'] = pd.to_datetime("today").year
    status = dbLastseason[(dbLastseason['season'] == dbLastseason['curr_year'] )]
    
    if len(status)<1:
        curr_year = dbLastseason['curr_year'][0]
        dbseason = dbLastseason['season'][0]
        years = list(range(curr_year, dbseason, -1))
        ...
        if not database_exists(engine.uri):
...
def upcoming_matches():
    ...
    upcoming_matches['xG'] = ''
    upcoming_matches['xGA'] = ''
    upcoming_matches['mod1'] = ''
    upcoming_matches['mod2'] = ''

    upcoming_matches_df = upcoming_matches.iloc[0:]
